...
export const employesApi = {
  ...
  search: (keyword: string) => api.get<ApiResponse<any[]>>('/employes/search', { params: { keyword } }),
};
...
export const demandesCongesApi = {
  ...
  updateStatut: (id: number, data: any) => api.put<ApiResponse<any>>(`/demandes-conges/${id}/statut`, data),
  getEnAttente: () => api.get<ApiResponse<any[]>>('/demandes-conges/en-attente'),
};

// ICA (Prime)
export const icaApi = {
  getSuiviComplet: (annee?: number) => api.get<ApiResponse<any[]>>('/ica/suivi', { params: { annee } }),
  getSuiviParSousDirection: (sousDirectionId: number, annee?: number) => 
    api.get<ApiResponse<any[]>>(`/ica/suivi/sous-direction/${sousDirectionId}`, { params: { annee } }),
  getSuiviParService: (serviceId: number, annee?: number) => 
    api.get<ApiResponse<any[]>>(`/ica/suivi/service/${serviceId}`, { params: { annee } }),
  getStatistiques: (annee?: number) => api.get<ApiResponse<any>>('/ica/statistiques', { params: { annee } }),
  getSuiviEmploye: (employeId: number, annee?: number) => 
    api.get<ApiResponse<any>>(`/ica/employe/${employeId}`, { params: { annee } }),
  getEligibles: (annee?: number) => api.get<ApiResponse<any[]>>('/ica/eligibles', { params: { annee } }),
};
...
export const historiqueCongesApi = {
  ...
  getById: (id: number) => api.get<ApiResponse<any>>(`/historique-conges/${id}`),
  create: (data: any) => api.post<ApiResponse<any>>('/historique-conges', data),
  update: (id: number, data: any) => api.put<ApiResponse<any>>(`/historique-conges/${id}`, data),
  delete: (id: number) => api.delete<ApiResponse<void>>(`/historique-conges/${id}`),
  getByEmployeId: (employeId: number) => api.get<ApiResponse<any[]>>(`/historique-conges/employe/${employeId}`),
  getByAnnee: (annee: number) => api.get<ApiResponse<any[]>>(`/historique-conges/annee/${annee}`),
  recalculerICA: (id: number) => api.put<ApiResponse<any>>(`/historique-conges/${id}/recalculer-ica`),
  ajuster: (id: number, ajustement: number, remarque?: string) => 
    api.put<ApiResponse<any>>(`/historique-conges/${id}/ajuster`, null, { params: { ajustement, remarque } }),
};
